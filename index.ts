...
import { Dispatch, SetStateAction, useEffect, type useState } from "react";

const eqSet = <T>(A: Set<T>, B: Set<T>) =>
  A.size === B.size && [...A].every((x) => B.has(x));

const normalizeParam = (query: NextRouter["query"], param: string) => {
...
type UseStateReturns<T> = [T, Dispatch<SetStateAction<T>>];
type AllowedInputs = UseStateReturns<string[]> | UseStateReturns<string>;
...
export const useStatesToNextQuery = (
  router: NextRouter,
  useStates: Record<string, AllowedInputs>
) => {
      ...
      if (hasChange({ stateValue, param })) {
        if (param.length > 0) {
          if (Array.isArray(stateValue)) {
            // @ts-expect-error this typing is a bit off
            setStateValue(param);
          } else {
            // @ts-expect-error this typing is a bit off
